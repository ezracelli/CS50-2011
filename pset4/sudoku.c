...
bool did_win(void);
void draw_win_logo(void);
void lock_board(void);
bool check_for_error(void);
bool board_filled(void);
bool error_in_row(void);
bool error_in_column(void);
bool error_in_box(void);
...
// following four functions move the cursor
bool
move_up(void)
{
    if (g.y > 0) {
...
did_win(void)
{
    if (!check_for_error() && board_filled()) {
        return true;
    }
    return false;
}

// ensures current board cannot be edited
void
lock_board(void)
{
    for (int i = 0; i < 9; i++) {
...
// if any errors are found, display banner, reset cursor, and return true
bool
check_for_error(void)
{
    if(error_in_row()) {
        show_banner("ERROR IN ROW");
        show_cursor();
        return true;
    } else if (error_in_column()) {
        show_banner("ERROR IN COLUMN");
        show_cursor();
        return true;
    } else if (error_in_box()) {
        show_banner("ERROR IN BOX");
        show_cursor();
        return true;
    } else {
        return false;
...
bool
board_filled(void) {
    // if any value in board is 0, return false
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (g.board[i][j] == 0) {
                return false;
    ...
    // else, the board is filled. return true
    return true;
}

// following three functions check for errors in row, column, and box, respectively
bool
error_in_row(void) {
    // if any value in row is the same as another, return true
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (i == j) {
                // if comparing the same value, skip comparison
                continue;
            } else if (g.board[g.y][i] == 0 || g.board[g.y][j] == 0) {
                // else if either value is 0, skip comparison
                continue;
            } else if (g.board[g.y][i] == g.board[g.y][j]) {
                // if non-identical, non-zero values are the same, there is an error
                return true;
            }
        }
    }
...
bool
error_in_column(void) {
    // if any value in column is the same as another, return true
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (i == j) {
                // if comparing the same value, skip comparison
                continue;
            } else if (g.board[i][g.x] == 0 || g.board[j][g.x] == 0) {
                // else if either value is 0, skip comparison
                continue;
            } else if (g.board[i][g.x] == g.board[j][g.x]) {
                // if non-identical, non-zero values are the same, there is an error
                return true;
            }
        }
...
bool
error_in_box(void) {
    // declare y, x values for both i and j in for loop below
    int iy;
    int ix;
    int jy;
    int jx;
    ...
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            ...
            if (i == j) {
                // if comparing the same value, skip comparison
                continue;
            } else if (g.board[iy][ix] == 0 || g.board[jy][jx] == 0) {
                // else if either value is 0, skip comparison
                continue;
            } else if (g.board[iy][ix] == g.board[jy][jx]) {
                // if non-identical, non-zero values are the same, there is an error
                return true;
            }
        }
